...
async function boot(): Promise<void> {
  ...
  server.listen(config.port, () => console.log(`creavy-quote listening on :${config.port} (${config.env}) — origin ${config.allowedOrigin}, turnstile ${config.turnstile.enabled ? "on" : "off"}`));
}
